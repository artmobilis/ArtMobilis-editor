...
.directive('channelRender', ['DataManagerSvc', function(DataManagerSvc) {
  return {
    ...
    link: function(scope, element, attr) {
      ...
      _camera.position.z = Math.max(_camera_distance, 1) * 1.5;

      _scene.add(_contents_meshes);

      attr.$observe('channel', function(attr_channel_id) {
        if (_marker_mesh)
          _scene.remove(_marker_mesh);
      ...
      scope.$on('$destroy', function() {
        ...
        _canvas.removeEventListener('touchstart', TouchStart, false);
        _canvas.removeEventListener('touchend', TouchEnd, false);
        _canvas.removeEventListener('touchcancel', TouchEnd, false);
        _canvas.removeEventListener('touchleave', TouchEnd, false);
        _canvas.removeEventListener('touchmove', TouchMove, false);

        _canvas.removeEventListener('mousedown', MouseStart, false);
        _canvas.removeEventListener('mousemove', MouseMove, false);
        _canvas.removeEventListener('mouseup', MouseEnd, false);

        _canvas.removeEventListener('mousewheel', MouseWheel, false);
        _canvas.removeEventListener('DOMMouseScroll', MouseWheel, false);
      });
